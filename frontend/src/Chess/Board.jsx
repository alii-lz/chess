...
const Piece = ({ Component, position, color, board, setValidMoves }) => {
  const [{ isDragging }, drag] = useDrag({
    ...
    item: () => {
      ...
      if (piece && piece.color === (color === "white" ? "white" : "black")) {
...
const Square = ({
  ...
  const [{ isOver }, drop] = useDrop({
    accept:
      ItemTypes.PIECE /* Only makes items of type {ItemTypes.PIECE} draggable */,
    drop: (item) => {
      if (isValidMove) {
...
const Board = () => {
  ...
  const handleSquareClick = React.useCallback(
    ...
    [board, validMoves, selectedPiecePosition, whiteTurn, dispatch, movePiece]
  );
